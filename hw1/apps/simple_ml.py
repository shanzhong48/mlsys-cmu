...
def softmax_loss(Z, y_one_hot):
    ...
    """
    ### BEGIN YOUR SOLUTION
    return ndl.summation(ndl.log(ndl.summation(ndl.exp(Z), axes=(1,))) - ndl.summation(ndl.multiply(Z, y_one_hot), axes=(1,)))/Z.shape[0]
    ### END YOUR SOLUTION
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(0, X.shape[0], batch):
        ...
        Z = ndl.matmul(ndl.relu(ndl.matmul(ndl.Tensor(x), W1)), W2)
        
        Iy = np.zeros((batch, W2.shape[1]))
        Iy[np.arange(batch), yy] = 1.0

        # tensor_x = ndl.Tensor(x)
        tensor_y = ndl.Tensor(Iy)
        ...
        W1 = ndl.Tensor(W1.numpy() - lr * update1)
        W2 = ndl.Tensor(W2.numpy() - lr * update2)
    return W1, W2

    # return ndl.Tensor(new_W1), ndl.Tensor(new_W2)
