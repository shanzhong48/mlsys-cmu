...
class Linear(Module):
    ...
    def forward(self, X: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        return ops.matmul(X, self.weight) + self.bias.broadcast_to((X.shape[0], self.out_features))
        ### END YOUR SOLUTION
...
class SoftmaxLoss(Module):
    def forward(self, logits: Tensor, y: Tensor):
        ### BEGIN YOUR SOLUTION
        one_hot_y = init.one_hot(logits.shape[1], y, requires_grad=True)
        if len(logits.shape) > 2:
            one_hot_y = one_hot_y.reshape((logits.shape[0], logits.shape[1], 1))
        return ops.summation(ops.logsumexp(logits, (1,))/logits.shape[0]) - ops.summation(one_hot_y.broadcast_to(logits.shape) * logits / logits.shape[0])
        ### END YOUR SOLUTION
...
class BatchNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            # normalized_x = (x - mean.reshape((1,self.dim)).broadcast_to(x.shape))/(var.reshape((1,self.dim)).broadcast_to(x.shape) + self.eps)**0.5
            normalized_x = (x - mean.broadcast_to(x.shape))/(var.broadcast_to(x.shape) + self.eps)**0.5
        else:
            normalized_x = (x - self.running_mean.broadcast_to(x.shape))/(self.running_var.broadcast_to(x.shape) + self.eps)**0.5
