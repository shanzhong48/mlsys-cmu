...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, X.shape[0], batch):
        Z = np.exp(np.matmul(X[i:i+batch], theta))/(np.sum(np.exp(np.matmul(X[i:i+batch], theta)), axis=1)[:,None])
        Iy = np.zeros((batch, theta.shape[1]))
        Iy[np.arange(batch), y[i:i+batch]] = 1
        theta -= lr / batch * np.matmul(X[i:i+batch].T, (Z - Iy))
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, X.shape[0], batch):
        ...
        G2 = np.exp(Z1 @ W2)/np.sum(np.exp(Z1 @ W2), axis=1)[:,None] - Iy
        Zpos = np.zeros(Z1.shape)
        Zpos[Z1>0] = 1
        G1 = Zpos*(G2 @ W2.T)

        update1 = x.T @ G1 / batch
        update2 = Z1.T @ G2 /batch
